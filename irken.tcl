...
namespace eval ::irken {
    ...
    proc connect {serverid} {
        ...
        set chan [if {$insecure} {socket -async $host $port} {if {$certfile ne ""} {tls::socket -certfile $certfile -async $host $port } {tls::socket -async $host $port }}]
    ...
    proc parseline {line} {
        ...
        if {[dict exists $msg tags time]} {
            dict set msg time [clock scan [regsub {\.\d+} [dict get $msg tags time] ""]]
        } else {
